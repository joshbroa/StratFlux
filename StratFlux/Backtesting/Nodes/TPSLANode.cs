...
namespace StratFlux.Backtesting.Nodes
{
    public class TPSLANode : Node
    {
        ...
        public TPSLANode(int id, NodeInput[] inputs, NodeOutput[] outputs, JObject jsonData)
            : base(id, inputs, outputs, jsonData, NodeType.Action, NodeName.TPSLA) { }
...
#nullable enable
        ...
        // Three values are needed: boolean which decides if trade happens, take profit amount, and stop loss amount
        ...
        public override Order? EvaluateAction(decimal[] numInputs, bool[] boolInputs)
        {
            if (boolInputs[0])
            {
                ...
                return new Order(_shareAmount, numInputs[0], numInputs[1], false);
            }
            else
            {
