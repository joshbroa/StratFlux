...
namespace StratFlux.Backtesting.Nodes
{
    public class VolumeNode : Node
    {
        ...
        public VolumeNode(int id, NodeInput[] inputs, NodeOutput[] outputs, JObject jsonData)
            : base(id, inputs, outputs, jsonData, NodeType.Value, NodeName.Volume)
        {
            _hasStarted = false;
        }
...
#nullable enable
        ...
        {
            _currentValue = requiredData[0].Volume;
            _hasStarted = true;
        }
        ...
        // Volume value is updated to volume value of new tick
        public override void UpdateValue(IBar[] requiredData)
        {
            _currentValue = requiredData[0].Volume;
        }

        // To evalutate value of node, stored volume value is returned
