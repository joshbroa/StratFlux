...
namespace StratFlux.Backtesting.Nodes
{
    public class BOLUNode : Node
    {
        ...
        public BOLUNode(int id, NodeInput[] inputs, NodeOutput[] outputs, JObject jsonData)
            : base(id, inputs, outputs, jsonData, NodeType.Value, NodeName.BOLU)
        {
            _hasStarted = false;
        }
...
#nullable enable
        ...
        // This follows the standard deviation equation for the standard deviation and then uses that value to get the upper bollinger band based on user's parameters
        public override void CreateFirstValue(IBar[] requiredData)
        {
            ...
            // Using standard deviation, upper bollinger band can be created
