...
namespace StratFlux.Backtesting.Nodes
{
    public class LowNode : Node
    {
        ...
        public LowNode(int id, NodeInput[] inputs, NodeOutput[] outputs, JObject jsonData)
            : base(id, inputs, outputs, jsonData, NodeType.Value, NodeName.Low)
        {
            _hasStarted = false;
        }
...
#nullable enable
        ...
        {
            _currentValue = requiredData[0].Low;
            _hasStarted = true;
        }
        ...
        // Low value is updated to low value of new tick
        public override void UpdateValue(IBar[] requiredData)
        {
            _currentValue = requiredData[0].Low;
        }

        // To evalutate value of node, stored low value is returned
